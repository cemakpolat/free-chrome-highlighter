...
class PluginLoader {
  ...
  _registerStoragePlugins() {
    ...
    // Chrome Sync (requires a Google account signed into Chrome; 100KB limit)
    this.registry.register('storage', 'chrome-sync', new ChromeSyncPlugin());
...
class LocalStoragePlugin {
  ...
  async save(domainKey, highlights) {
    ...
    // Preserve Phase 1 nested format so highlighter-service stays compatible
    const existing = await chrome.storage.local.get(key);
    const prev = existing[key];
    const data = (prev && !Array.isArray(prev))
      ? { ...prev, highlights, lastModified: Date.now() }
      : { highlights, lastModified: Date.now() };
  ...
  async load(domainKey) {
    ...
    if (!val) return [];
    // Handle both Phase 1 nested format and flat array
    return Array.isArray(val) ? val : (val.highlights || []);
  }
...
class ChromeSyncPlugin {
  static get metadata() {
    return {
      ...
      version: '1.0.0',
      category: 'storage',
      description: 'Syncs highlights across all your Chrome browsers using your Google account. No extra setup needed — uses Chrome\'s built-in sync. Note: 100 KB per-item limit applies.',
      auth: null,
      isBuiltIn: true,
